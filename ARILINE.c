...
int main()
{
    ...
    do
    {
        ...
        case 2:
            printf("C_name\t C_id\t S_from\t S_to\t Flight_time\t Date\t Fare\t Class\n");
            printf("%s\t %d\t %s\t %s\t %d/%d/%d\t %s\t %s\n", p.s.d.c.name, p.s.d.c.id, p.s.str_from, p.s.str_to, p.s.d.time, p.s.d.dd, p.s.d.mm, p.s.d.yyyy, p.fare, p.class);
            break;
        ...
        printf("if continue plz enter 1 =");
        scanf("%d", &ch1);
    } while (ch1 == 1);
}
void customer()
